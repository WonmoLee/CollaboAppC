...
class AppUpdater extends _events().EventEmitter {
  ...
  getUpdateInfoAndProvider() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (_this4.clientPromise == null) {
        _this4.clientPromise = _this4.configOnDisk.value.then(it => (0, _providerFactory().createClient)(it, _this4, _this4.createProviderRuntimeOptions()));
      }
      ...
      return {
        info: yield client.getLatestVersion(),
        provider: client
      };
    })();
  }
  ...
  doCheckForUpdates() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const result = yield _this5.getUpdateInfoAndProvider();
      ...
      _this5.updateInfoAndProvider = result;
  ...
  loadUpdateConfig() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      if (_this6._appUpdateConfigPath == null) {
        _this6._appUpdateConfigPath = _this6.app.appUpdateConfigPath;
      }
  ...
  getOrCreateStagingUserId() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const file = path.join(_this7.app.userDataPath, ".updaterId");
  ...
  getOrCreateDownloadHelper() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      let result = _this8.downloadedUpdateHelper;

      if (result == null) {
        const dirName = (yield _this8.configOnDisk.value).updaterCacheDirName;
        const logger = _this8._logger;

        if (dirName == null) {
          logger.error("updaterCacheDirName is not specified in app-update.yml Was app build using at least electron-builder 20.34.0?");
        }

        const cacheDir = path.join(_this8.app.baseCachePath, dirName || _this8.app.name);
        ...
        _this8.downloadedUpdateHelper = result;
      }
  ...
  executeDownload(taskOptions) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (_this9.listenerCount(_main().DOWNLOAD_PROGRESS) > 0) {
        downloadOptions.onProgress = it => _this9.emit(_main().DOWNLOAD_PROGRESS, it);
      }
      ...
      const downloadedUpdateHelper = yield _this9.getOrCreateDownloadHelper();
      ...
      function () {
        var _ref = (0, _bluebirdLst().coroutine)(function* (isSaveCache) {
          ...
          if (isSaveCache) {
            yield downloadedUpdateHelper.cacheUpdateInfo(updateFileName);
          }

          yield taskOptions.done(Object.assign({}, updateInfo, {
      ...
      const log = _this9._logger;
      const cachedUpdateFile = yield downloadedUpdateHelper.validateDownloadedPath(updateFile, updateInfo, fileInfo, log);
      ...
      function () {
        var _ref2 = (0, _bluebirdLst().coroutine)(function* () {
          yield downloadedUpdateHelper.clear().catch(() => {// ignore
          });
      ...
      const tempUpdateFile = yield (0, _DownloadedUpdateHelper().createTempUpdateFile)(`temp-${updateFileName}`, cacheDir, log);
      ...
      try {
        ...
        if (e instanceof _builderUtilRuntime().CancellationError) {
          ...
          _this9.emit("update-cancelled", updateInfo);
        }
